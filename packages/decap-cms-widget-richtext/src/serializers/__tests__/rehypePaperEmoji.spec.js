...
describe('rehypePaperEmoji', () => {
  ...
  it('recursively replaces emoji img children nested multiple levels deep', () => {
    const emojiImg = makeImgNode({ dataEmojiCh: '🔥' });
    const inner = makeElementNode('span', [emojiImg]);
    const outer = makeElementNode('p', [inner]);
    const result = transform(outer);
    expect(result.children[0].children).toEqual([{ type: 'text', value: '🔥' }]);
  });

  it('leaves nodes without children untouched', () => {
    const node = { type: 'text', value: 'plain text' };
    const result = transform(node);
    expect(result).toBe(node);
  });
});
