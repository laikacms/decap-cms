...
export const ExtractAuthorizationBearerToken = (header: string | null | undefined): string | undefined => {
  if (!header) return undefined;
  const match = header.match(AuthorizationHeaderBearerRegex);
  return match ? match[1] : undefined;
}
export const ExtractAuthorizationApiKey = (header: string | null | undefined): string | undefined => {
  if (!header) return undefined;
