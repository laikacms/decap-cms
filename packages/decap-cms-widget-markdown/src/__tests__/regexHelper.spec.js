...
describe('replaceWhen', () => {
  ...
  it('returns the input run through replaceFn when there are no matches', () => {
    const result = replaceWhen(/xyz/g, upperCase, 'foo bar baz', false);
    expect(result).toBe('FOO BAR BAZ');
  });

  it('applies replaceFn to the whole string when there are no matches, regardless of invertMatchPattern', () => {
  ...
  it('handles adjacent matches with no text between them', () => {
    const result = replaceWhen(/foo|bar/g, upperCase, 'foobar baz', false);
    expect(result).toBe('FOOBAR baz');
  });

  it('handles multiple non-adjacent matches with unmatched text before, between, and after', () => {
  ...
  it('handles a match at the very end of the string', () => {
    const result = replaceWhen(/baz/g, upperCase, 'foo bar baz', false);
    expect(result).toBe('foo bar BAZ');
  });

  it('replaces non-matching substrings instead of matches when invertMatchPattern is true', () => {
