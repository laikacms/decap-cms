...
export default function remarkToRehypeShortcodes({ plugins, getAsset, resolveWidget, toHtml }) {
  ...
  function transform(root) {
    const transformedChildren = map(root.children, node =>
      processShortcodes(node, { plugins, getAsset, resolveWidget, toHtml }),
    );
 ...
 */
export function processShortcodes(node, { plugins, getAsset, resolveWidget, toHtml }) {
  /**
   ...
   */
  const value = getPreview(plugin, shortcodeData, { getAsset, resolveWidget, toHtml });
 ...
 */
function getPreview(plugin, shortcodeData, { getAsset, resolveWidget, toHtml }) {
