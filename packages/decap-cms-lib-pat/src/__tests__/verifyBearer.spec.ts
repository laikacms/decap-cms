import { hashToken, mintPersonalAccessToken } from '../token';
import { resolveBearer } from '../verifyBearer';
...
describe('resolveBearer', () => {
  ...
  it('never calls verifySessionToken for a PAT-shaped bearer', async () => {
    ...
    const verifySessionToken = jest.fn();
  ...
  it('looks PATs up by hash only, never by plaintext token', async () => {
    ...
    const lookupPatByHash = jest.fn(async (hash: string) =>
