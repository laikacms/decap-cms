...
/**
 * Generates a new opaque PAT bearer string, e.g. `lk_pat_Ab3fG...`.
 * Never persisted in plaintext; only `hashToken(token)` is stored.
 */
 ...
 * builds the record consumers persist. `scopes` must be a non-empty,
 * non-admin-widening subset of the granting user's own scopes -- enforcing
 * that "PATs issue a subset of a user's role-scopes" is the caller's job
 * (it requires knowing the requesting user's own scopes), this function
 * only refuses structurally invalid input.
 */
