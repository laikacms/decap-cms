import { ADMIN_SCOPE, expandScopes } from './scopes';
import { hashToken, isPatToken } from './token';

import type { AuthContext, PatRecord, SessionVerificationResult } from './types';

export interface ResolveBearerDeps {
  /**
   * Validates a non-PAT bearer against the existing OAuth session-token
   * seam (unchanged). Return `null`/`undefined` for an invalid/expired
   * token. Omitting `scopes` on the result means "full admin", matching
   * today's behaviour where any authenticated session is full-admin.
   */
...
/**
 * The single seam every consumer (REST API, MCP endpoints, ...) should call
 * to turn an `Authorization: Bearer <token>` header into `{ user, scopes }`.
 * A session token resolves to full scopes; a PAT resolves to its granted
 * subset. Returns `null` for anything invalid, revoked, or expired -- never
 * throws for bad input, so callers can treat `null` uniformly as 401.
 */
