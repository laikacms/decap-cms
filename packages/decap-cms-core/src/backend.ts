...
export class Backend {
  ...
  constructor(implementation: Implementation, { backendName, authStore, config }: BackendOptions) {
    ...
    this.implementation = implementation.init(this.config, {
      ...
      initialWorkflowStatus: status.DRAFT,
    });
      ...
      .map(async collection => {
        ...
        if (collection.type === FILES) {
          collection.files?.forEach(f => {
            const topLevelFields = f.fields.map(field => field.name);
  ...
  async persistEntry({
    ...
    if (updatedEntity && (updatedEntity as Partial<Entry>).data === undefined) {
      entryDraft = { ...draft, entry: { ...draft.entry, data: updatedEntity } };
    } else {
      entryDraft = updatedEntity ? { ...draft, entry: updatedEntity } : draft;
    }
  ...
  fieldsOrder(collection: Collection, entry: EntryMap) {
    ...
    const file = (files || []).find(f => f.name === entry.slug);
