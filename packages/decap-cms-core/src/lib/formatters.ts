...
export function previewUrlFormatter(
  ...
  function getPathTemplate() {
    return file?.get('preview_path') ?? collection.preview_path;
  }

  function getDateField() {
    return file?.get('preview_path_date_field') ?? collection.preview_path_date_field;
  }
  ...
  fields = addFileTemplateFields(entry.get('path'), fields, collection.folder);
  ...
  const previewPathPreserveSlashes =
    file?.get('preview_path_preserve_slashes') ?? collection.preview_path_preserve_slashes;
  const preserveSlashes = !!(previewPathPreserveSlashes ?? collection.nested !== undefined);
