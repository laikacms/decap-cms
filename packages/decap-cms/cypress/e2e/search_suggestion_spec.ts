...
const search = (term: string, collection: string) => {
...
const assertSearchHeading = (title: string) => {
  cy.get('[class*=SearchResultHeading]').should('have.text', title);
};

const assertSearchResult = (text: string, collection?: string) => {
  cy.get('[class*=ListCardLink] h2').contains(collection ?? text);
};

const assertNotInSearch = (text: string) => {
