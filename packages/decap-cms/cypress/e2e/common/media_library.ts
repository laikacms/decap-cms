...
function logFailPoint(label: string): void {
  cy.then(() => {
    const currentWindow = Cypress.state('window') as Window | undefined;
...
function waitForEntryToLoad(): void {
...
interface MediaLibraryOptions {
  entries: Entry[];
  getUser?: () => TaskDataResult['user'];
}

export default function({ entries, getUser }: MediaLibraryOptions): void {
  beforeEach(() => {
