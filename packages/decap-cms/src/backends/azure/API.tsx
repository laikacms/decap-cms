...
import type { ApiRequest, CmsAssetProxy, CmsDataFile, CmsPersistOptions } from '@/lib/util/index';
...
export default class API {
  ...
  withAzureFeatures = (req: ApiRequest) => {
    if (typeof req !== 'string' && req.params && API_VERSION in req.params) {
      return req;
    }
  ...
  request = (req: ApiRequest): Promise<Response> => {
    try {
      ...
      throw new APIError('An unknown error occurred', null, API_NAME);
    }
  };

  responseToJSON = responseParser<'json'>({ format: 'json', apiName: API_NAME });
  responseToBlob = responseParser<'blob'>({ format: 'blob', apiName: API_NAME });
  responseToText = responseParser<'text'>({ format: 'text', apiName: API_NAME });
    ...
    const items = await Promise.all(
      files.map(async file => {
        const [base64Content, fileExists] = await Promise.all([
          result(file, 'toBase64', partial(this.toBase64, (file as CmsDataFile).raw)),
  ...
  async deleteFiles(paths: string[], comment: string) {
    ...
    const changes = paths.map(path => getChangeItem({ action: AzureCommitChangeType.DELETE, path }));
    const commits = [{ comment, changes }];
    const push = {
      refUpdates: [refUpdate],
      commits,
  ...
  async editorialWorkflowGit(
    ...
    if (!unpublished) {
      ...
      const items = await this.getCommitItems(files, branch, hasSubfolders);
