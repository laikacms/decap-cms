...
enabled (see below). Wired up via the separate `@laikacms/decap-cms/backends/github/graphql` entry
point (which calls `registerGraphQLAPI`) so the GraphQL client libraries stay optional peer
dependencies.
