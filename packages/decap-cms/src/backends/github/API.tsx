...
export default class API {
  ...
  async storeMetadata(key: string, data: Metadata) {
    ...
    if (!this._metadataSemaphore) {
      this._metadataSemaphore = createSemaphore(1);
    }
  ...
  async getPullRequests(
    ...
    const pullRequests: Endpoints['GET /repos/{owner}/{repo}/pulls']['response']['data'] = await this.requestAllPages(
  ...
  async getBranchPullRequest(branch: string) {
    if (this.useOpenAuthoring) {
      ...
      const pullRequests = await this.getPullRequests(
        ...
        pr => withCmsLabel(pr, this.cmsLabelPrefix),
      );
  ...
  async getPullRequestCommits(number: number) {
    ...
    try {
      const commits: Endpoints['GET /repos/{owner}/{repo}/pulls/{pull_number}/commits']['response']['data'] = await this
        .request(`${this.originRepoURL}/pulls/${number}/commits`);
      return commits;
    } catch (e: unknown) {
      console.log(e);
      return [];
  ...
  async readFile(
    ...
    // Blobs are immutable per sha, so cache them in localForage keyed on the sha,
    // matching the gitlab/bitbucket/azure backends.
    const fetchContent = () => this.fetchBlobContent({ sha: sha as string, repoURL, parseText });
    const content = await readFileFromCache(sha, fetchContent, localForage, parseText);
    return content;
  }
  ...
  async readFileMetadata(path: string, sha: string | null | undefined) {
    const fetchFileMetadata = async () => {
      try {
        const result: Endpoints['GET /repos/{owner}/{repo}/commits']['response']['data'] = await this.request(
  ...
  async fetchBlobContent({ sha, repoURL, parseText }: BlobArgs) {
    const result: Endpoints['GET /repos/{owner}/{repo}/git/blobs/{file_sha}']['response']['data'] = await this.request(
  ...
  async listFiles(
    ...
    try {
      const result: Endpoints['GET /repos/{owner}/{repo}/git/trees/{tree_sha}']['response']['data'] = await this
        .request(`${repoURL}/git/trees/${branch}:${folder}`, {
  ...
  async migrateToVersion1(pullRequest: GitHubPull, metadata: Metadata) {
    ...
    const pr = (await this.getPullRequests(newBranchName, PullRequestState.All, () => true))[0]
  ...
  async listUnpublishedBranches() {
    ...
    if (this.useOpenAuthoring) {
      ...
      const cmsBranches: Endpoints['GET /repos/{owner}/{repo}/git/matching-refs/{ref}']['response']['data'] = await this
  ...
  async persistFiles(
    ...
    if (!options.useWorkflow) {
      return this.getDefaultBranch()
        .then(branchData => this.updateTree(branchData.commit.sha, files as { sha: string, path: string }[]))
  ...
  async getFileSha(path: string, { repoURL = this.repoURL, branch = this.branch } = {}) {
    ...
    const result: Endpoints['GET /repos/{owner}/{repo}/git/trees/{tree_sha}']['response']['data'] = await this.request(
      fileDataURL,
    );
  ...
  async getDifferences(from: string, to: string) {
    ...
    for (let i = 1; i <= attempts; i++) {
      try {
        const result: Endpoints['GET /repos/{owner}/{repo}/compare/{base}...{head}']['response']['data'] = await this
          .request(`${this.originRepoURL}/compare/${from}...${to}`);
        return result;
      } catch (e: unknown) {
        if (i === attempts) {
  ...
  async createRef(type: string, name: string, sha: string) {
    const result: Endpoints['POST /repos/{owner}/{repo}/git/refs']['response']['data'] = await this.request(
      `${this.repoURL}/git/refs`,
      {
        method: 'POST',
  ...
  async patchRef(type: string, name: string, sha: string, opts: { force?: boolean } = {}) {
    ...
    const result: Endpoints['PATCH /repos/{owner}/{repo}/git/refs/{ref}']['response']['data'] = await this.request(
  ...
  async getBranch(branch: string) {
    const result: Endpoints['GET /repos/{owner}/{repo}/branches/{branch}']['response']['data'] = await this.request(
  ...
  async createPR(title: string, head: string) {
    const result: Endpoints['POST /repos/{owner}/{repo}/pulls']['response']['data'] = await this.request(
      `${this.originRepoURL}/pulls`,
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  async openPR(number: number) {
    ...
    const result: Endpoints['PATCH /repos/{owner}/{repo}/pulls/{pull_number}']['response']['data'] = await this.request(
  ...
  async mergePR(pullrequest: GitHubPull) {
    ...
    try {
      const result: Endpoints['PUT /repos/{owner}/{repo}/pulls/{pull_number}/merge']['response']['data'] = await this
        .request(`${this.originRepoURL}/pulls/${pullrequest.number}/merge`, {
          method: 'PUT',
          body: JSON.stringify({
  ...
  async createTree(baseSha: string, tree: TreeEntry[]) {
    const result: Endpoints['POST /repos/{owner}/{repo}/git/trees']['response']['data'] = await this.request(
      `${this.repoURL}/git/trees`,
      {
        method: 'POST',
  ...
  async createCommit(
    ...
    const result: Endpoints['POST /repos/{owner}/{repo}/git/commits']['response']['data'] = await this.request(
