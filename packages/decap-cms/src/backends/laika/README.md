...
| `acceptRoles`                      | `string[]` | no                | Role allowlist enforced in `authenticate()`. When set to a **non-empty** array, the `role` attribute the `/session` endpoint returns for the authenticated user must be one of these values, or `authenticate()` throws an `APIError` (status `403`) — no repositories are initialized and no session is persisted to `sessionStorage`. `restoreUser()` re-runs this check on every page load (it delegates to `authenticate()`). Omit the key, or set it to an empty array (`[]`), to admit any authenticated session regardless of role — both are treated identically to not enforcing any role check, same as before this option existed. A session with no `role` attribute is rejected whenever `acceptRoles` is a non-empty array. |
