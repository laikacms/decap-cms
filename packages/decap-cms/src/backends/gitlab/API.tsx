import { flow, partial, result, trimStart } from 'lodash-es';
...
import type { ApiRequest, CmsAssetProxy, CmsDataFile, CmsPersistOptions, FetchError } from '@/lib/util/index';
...
export type FileEntry = { id: string, type: string, path: string, name: string };
...
export default class API {
  ...
  request = async (req: ApiRequest): Promise<Response> => {
    try {
      ...
      throw new APIError((err as FetchError).message, null, API_NAME);
    }
  };
  ...
  hasWriteAccess = async () => {
    const { shared_with_groups: sharedWithGroups, permissions }: GitLabRepo = await this.requestJSON(this.repoURL);
  ...
  fetchCursor = (req: ApiRequest) =>
    flow([unsentRequest.withMethod('HEAD'), this.request, promiseThen(this.getCursor)])(req);
