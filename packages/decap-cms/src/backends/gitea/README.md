...
- The Gitea backend does not support the [editorial workflow](https://www.decapcms.org/docs/beta-features/#gitlab-and-bitbucket-editorial-workflow-support). Setting `publish_mode: editorial_workflow` with `name: gitea` throws `'The Gitea backend does not support editorial workflow.'` at initialization.
- Authentication uses the PKCE OAuth flow (`login/oauth/authorize` / `login/oauth/access_token` against `base_url`), not a personal access token flow.
