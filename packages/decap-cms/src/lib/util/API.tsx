...
interface API {
  rateLimiter?: AsyncLock | undefined;
...
export async function requestWithBackoff(
  ...
  try {
    ...
    const error = err as { message?: string, resetSeconds?: number };
    if (attempt > 5 || error.message === "Can't refresh access token when using implicit auth") {
      ...
      if (!api.rateLimiter) {
        ...
        console.log(
          `Pausing requests for ${timeout} ${attempt === 1 ? 'second' : 'seconds'} due to fetch failures:`,
          error.message,
        );
...
export async function apiRequest(
  ...
  try {
    ...
    return handleRequestError(error as FetchError, responseStatus, backend);
  }
}
