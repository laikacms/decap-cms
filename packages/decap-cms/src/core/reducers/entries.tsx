...
const entries = produce((state: Entries, action: EntriesAction) => {
  switch (action.type) {
    ...
    case ENTRY_PERSIST_SUCCESS: {
      const payload = action.payload as EntryPersistedPayload;
      const { collectionName, slug, entry } = payload;
      // Apply the saved entry to the store so lists reflect the save without a
      // refetch. New entries without a resolved path are skipped; their slug and
      // path are computed backend-side and arrive via the follow-up entry load.
      if (!entry?.path || payload.entrySlug !== slug) break;
      state.entities[`${collectionName}.${slug}`] = { ...entry, slug, isFetching: false };
      const ids = state.pages[collectionName]?.ids ?? [];
      if (!ids.includes(slug)) {
        state.pages[collectionName] = {
          ...(state.pages[collectionName] ?? { isFetching: false, page: 0, ids: [] }),
          ids: [slug, ...ids],
        };
      }
      break;
