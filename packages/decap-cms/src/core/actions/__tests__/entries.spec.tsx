...
describe('entries', () => {
  ...
  describe('persistEntry - missing required field notification (DCMS-2151)', () => {
    const collection = {
      name: 'posts',
      type: FOLDER,
      folder: '_posts',
    ...
    it('uses smart_count 1 when a single required field is missing', async () => {
      ...
      const actions = store.getActions();
      const notification = actions.find((action: any) => action.type === 'NOTIFICATION_SEND');
    ...
    it('omits smart_count for non-presence validation errors', async () => {
      const store = mockStore(
        makeState({
          title: [{ type: 'PATTERN', message: 'Title is invalid' }],
        }),
      );
      ...
      const actions = store.getActions();
      const notification = actions.find((action: any) => action.type === 'NOTIFICATION_SEND');
      expect(notification.payload.message).toEqual({ key: 'ui.toast.invalidField' });
    });
  });
