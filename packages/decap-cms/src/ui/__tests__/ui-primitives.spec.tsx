...
describe('editor UI primitives', () => {
  ...
  describe('Toggle (Base UI)', () => {
    it('toggles its pressed state and reports it', async () => {
      const user = userEvent.setup();
      const onPressedChange = vi.fn();
      render(
        <Toggle aria-label="Toggle link" onPressedChange={onPressedChange}>
          Link
        </Toggle>,
      );

      const button = screen.getByRole('button', { name: 'Toggle link' });
    ...
    it('stays in sync with a controlled pressed prop', async () => {
      const user = userEvent.setup();
      const onPressedChange = vi.fn();
      render(
        <Toggle aria-label="Toggle link" pressed={false} onPressedChange={onPressedChange}>
          Link
        </Toggle>,
      );
      ...
      expect(onPressedChange).toHaveBeenCalledWith(true);
      // Controlled: the parent did not update `pressed`, so it stays off.
      expect(button).toHaveAttribute('aria-pressed', 'false');
    });
  });
