...
// Mirrors laikacms Header.ExtractAuthorizationBearerToken (laikacms/core), inlined
// here so this package does not depend on laikacms (DCMS-492).
const AUTHORIZATION_BEARER_REGEX = /^Bearer\s+([A-Za-z0-9\-._~+/]+={0,2})$/;

function extractAuthorizationBearerToken(header: string | null | undefined): string | undefined {
  if (!header) return undefined;
  const match = header.match(AUTHORIZATION_BEARER_REGEX);
