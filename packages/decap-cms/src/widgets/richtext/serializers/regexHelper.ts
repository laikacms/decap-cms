 ...
 */
export function joinPatternSegments(patterns: RegExp[]): string {
 ...
 *
 * params:
 * matchPattern - a regular expression to check for matches
 * replaceFn - a replacement function that receives a matched substring and
 *   returns a replacement substring
 * text - the string to process
 * invertMatchPattern - if true, non-matching substrings are modified instead of
 *   matching substrings
 */
export function replaceWhen(
   ...
   * array.
   */
  function addSubstring(arr: Substring[], index: number, substring: string, match = false) {
    arr.push({ index, text: substring, match });
  }
   ...
   * internally).
   */
  function split(exp: RegExp, input: string, acc: Substring[]): Substring[] {
    /**
  ...
  if (text.length > nextIndex) {
    acc.push({ index: nextIndex, text: text.slice(nextIndex), match: false });
  }
