...
import { combinePatterns, joinPatternSegments, replaceWhen } from './regexHelper';
 ...
 * joined before use.
 */
const nonEscapePatterns: RegExp[][] = [
  /**
   * HTML Tags
   *
  ...
  [
    ...
    /**
     * Match closing tag via first capture group. Built from a string because
     * the backreference only resolves once the segments above are joined.
     */
    new RegExp('<\\/\\1>'),
  ],
];
 ...
 */
const joinedNonEscapePatterns = nonEscapePatterns.map(
  pattern => new RegExp(joinPatternSegments(pattern)),
);
 ...
 */
function escapeDelimiters(pattern: RegExp, text: string) {
  return text.replace(pattern, (match: string, start: string, end?: string) => {
 ...
 */
function escapeAll(text: string) {
  return escapePatterns.reduce((acc, pattern) => escapeDelimiters(pattern, acc), text);
}
 ...
 */
function escapeCommonChars(text: string) {
  /**
 ...
 */
function escapeLeadingChars(text: string) {
 ...
 */
function escapeAllChars(text: string) {
...
export default function remarkEscapeMarkdownEntities() {
  function transform(node: MdastNode, index?: number): MdastNode {
    /**
