...
import remarkEscapeMarkdownEntities from '@/widgets/richtext/serializers/remarkEscapeMarkdownEntities';

import type { MdastNode, MdastRoot } from '@/widgets/richtext/types';

function process(text: string) {
  const tree = u<MdastRoot>('root', [u<MdastNode>('text', text)]);
  const escapedMdast = unified().use(remarkEscapeMarkdownEntities).runSync<MdastRoot>(tree);
