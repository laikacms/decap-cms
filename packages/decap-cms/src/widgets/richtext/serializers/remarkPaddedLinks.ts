import { endsWith, find, findLast, flatMap, startsWith, trimEnd, trimStart } from 'lodash-es';
...
export default function remarkPaddedLinks() {
   ...
   */
  function getEdgeTextChild(node: MdastNode, end?: boolean): MdastNode | undefined {
    const findFn = end ? findLast : find;

    let edgeChildWithValue: MdastNode | undefined;

    /**
     * setEdgeChildWithValue checks a node and all of it's children deeply to
     * find a non-blank text value. When the text node is found, we set it in an
     * outside variable, as it may be deep in the tree and therefore wouldn't be
     * returned by `find`/`findLast`.
     */
    function setEdgeChildWithValue(child: MdastNode) {
  ...
  function transformChildren(node: MdastNode): MdastNode | MdastNode[] {
    ...
    const leadingWhitespaceNode = startsWith(text, ' ') ? getEdgeTextChild(node) : undefined;
    const trailingWhitespaceNode = endsWith(text, ' ') ? getEdgeTextChild(node, true) : undefined;
    ...
    /**
     * Create an array of nodes, with a space text node on whichever edges had
     * whitespace trimmed off.
     */
    const nodes: MdastNode[] = [];
    if (leadingWhitespaceNode) nodes.push(u<MdastNode>('text', ' '));
    nodes.push(node);
    if (trailingWhitespaceNode) nodes.push(u<MdastNode>('text', ' '));

    return nodes;
  }

  function transform(node: MdastNode): MdastNode {
    /**
