...
// Apps that want a smaller bundle can import from `@laikacms/decap-cms/app/bare`
// instead and pick only the registrations they actually need. This entry is a
// composition root — the only kind of module (besides `laika-app`) allowed to
// run registrations at load; everything else in the package is side-effect free.
...
// Re-export everything from `./bare` so `@laikacms/decap-cms` (and its `./app`
