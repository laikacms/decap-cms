...
describe('GitLfsClient.uploadResource', () => {
  ...
  it('sends an upload batch request with the pointer file objects', async () => {
    const client = makeClient(['**']);
...
describe('GitLfsClient.downloadResource', () => {
  ...
  it('sends a download batch request and fetches the returned href', async () => {
    const client = makeClient(['**']);
    const blob = {} as Blob;
    makeAuthorizedRequest.mockResolvedValueOnce(
      jsonResponse({
    ...
    mockFetchWithTimeout.mockResolvedValueOnce({ blob: () => Promise.resolve(blob) });
  ...
  it('throws when the batch response has no download action for the object', async () => {
    ...
    await expect(client.downloadResource(pointer)).rejects.toThrow(
      "Failed to resolve LFS download URL for object 'abc123'",
    );
    expect(mockFetchWithTimeout).not.toHaveBeenCalled();
  });
});
