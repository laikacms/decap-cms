...
- `package.json` / `pnpm-workspace.yaml` / `pnpm-lock.yaml` — dependency or workspace topology
  changes (the v4.beta dependency drops are operator-gated since they break downstream consumers).
- `.github/` — CI/workflow changes.
- `eslint.config.mjs`, `tsconfig*.json`, `vite.config.*` — toolchain/config changes.
