...
5's `CmsEntryValue` deletion has landed early (it was already unreferenced). Stage 4
(media-off-entry, workflow composition) and the remainder of stage 5 (`EntryValue`/`CmsEntry`
...
5. Deletions (`CmsEntryValue`, `EntryValue` and its variants and creators, `CmsEntry`), docs,
   downstream consumers, optional no-`as` lint rule. `CmsEntryValue` deleted (DCMS-1907); the rest
   is follow-up.
