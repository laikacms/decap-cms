...
`peerDependenciesMeta` block in `packages/decap-cms/package.json` from scratch.

This supersedes the 2026-06-08 record below, which was checked against the pre-`41aff10e5` peer
set and is now stale. This doc is a point-in-time snapshot re-run whenever the peerDependencies
block changes materially, not a continuously-maintained invariant — see "Note on doc lifecycle"
below.

## Peer set checked (current, post-41aff10e5)
...
`pnpm install` was run from a fresh worktree checkout of `origin/v4.beta`
(`git worktree add ... origin/v4.beta`), lockfile already in sync ("Lockfile is up to date,
resolution step is skipped"). Full stdout/stderr:
...
Packages: +1086
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Progress: resolved 1086, reused 1086, downloaded 0, added 254
Progress: resolved 1086, reused 1086, downloaded 0, added 543
Progress: resolved 1086, reused 1086, downloaded 0, added 698
Progress: resolved 1086, reused 1086, downloaded 0, added 1053
...
warnings for any peer in the current set (required or optional), and no `husky - install command
is DEPRECATED` notice this run (that lifecycle notice from the 2026-06-08 record did not
reappear).
...
`uploadcare-widget`, `uploadcare-widget-tab-effects`).

## Note on doc lifecycle

This doc is kept as a maintained record (not archived), since peerDependencies changes are
infrequent but do recur (this is now the second full rebuild of that block). Re-verify and bump
the "Verified on" date/commit whenever `packages/decap-cms/package.json`'s
