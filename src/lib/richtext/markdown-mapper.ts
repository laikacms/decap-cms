...
import { resolveBlockCodecs } from './formats';
import { createKeyGenerator } from './keys';

import type { PortableTextTypeRenderer } from '@portabletext/markdown';
import type { TypedObject } from '@portabletext/types';
import type { BlockData, BlockFormatCodec } from './blocks/types';
import type { FormatPack } from './formats';
  ...
  return {
    ...
    fromPortableText(doc: PortableTextDocument): string {
      ...
      for (const [id, codec] of resolveCodecs()) {
        types[id] = ({ value }) =>
          codec.serialize(stripPtMeta(value as unknown as Record<string, unknown>));
      }
