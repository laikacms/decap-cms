/* eslint-disable @emotion/no-vanilla -- this widget's styling is intentionally
   built on `@emotion/css` class-name generation (`cva`-style variants). */
/**
 * Styling helpers for the emotion-ported UI components.
 *
 * Replaces the upstream `cn` (clsx + tailwind-merge) and `cva` helpers:
