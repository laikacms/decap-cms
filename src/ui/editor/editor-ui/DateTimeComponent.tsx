...
import {
  ...
  setLocalTime,
  toDateInputValue,
} from '@/ui/editor/utils/date-time';
      ...
      <PopoverTrigger
        render={
          <Button
            ...
            css={[
              triggerClass,
              Boolean(textFormat & IS_BOLD) && boldClass,
              Boolean(textFormat & IS_ITALIC) && italicClass,
              Boolean(textFormat & IS_UNDERLINE) && underlineClass,
              Boolean(textFormat & IS_STRIKETHROUGH) && strikeClass,
              Boolean(textFormat & IS_HIGHLIGHT) && highlightClass,
            ]}
          >
            <CalendarIcon css={calendarIconClass} />
            {displayLabel ?? <span>Pick a date</span>}
          </Button>
        }
      />
      <PopoverContent css={contentClass} align="start">
        <Input
          type="date"
          aria-label="Date"
          min="1925-01-01"
        ...
        <div css={timeRowWrapper}>
          <div css={flexRow}>
            ...
            />
            <Label htmlFor={`include-time-${nodeKey}`} css={flexRow}>
              <Input
                type="time"
                value={timeValue}
                ...
                css={timeInputClass}
              />
              <span css={zoneTextClass}>{userTimeZone}</span>
            </Label>
          </div>
        </div>
      </PopoverContent>
