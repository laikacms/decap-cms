...
export default class Toolbar extends React.Component {
  static propTypes = {
    ...
    editorComponents: PropTypes.array,
  ...
  render() {
    ...
    const showEditorComponents = !editorComponents || editorComponents.length >= 1;

    const allPlugins = plugins
      ? Object.values(plugins).sort((a, b) => String(a.label).localeCompare(String(b.label)))
      : [];

    const pluginsList = editorComponents
      ? editorComponents.map(id => plugins?.[id]).filter(plugin => !!plugin)
      : allPlugins;
