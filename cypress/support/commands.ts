  ...
  cy.stub(win, 'fetch').callsFake((...args: [string, FetchOptions?]) => {
    ...
    if (routeIndex >= 0) {
      ...
      const fetchResponse: FetchResponse = {
        ...
        text: () => Promise.resolve(typeof route.response === 'string' ? route.response : JSON.stringify(route.response)),
        json: () => Promise.resolve(JSON.parse(typeof route.response === 'string' ? route.response : JSON.stringify(route.response))),
...
keyCommands.forEach(key => {
  ...
  Cypress.Commands.add(cmd as keyof Cypress.Chainable, { prevSubject: true }, (subject: JQuery<HTMLElement>, { shift, times = 1 }: { shift?: boolean; times?: number } = {}) => {
    const fn = (chain: Cypress.Chainable<JQuery<HTMLElement>>) => chain.type(`${shift ? '{shift}' : ''}{${keyName}}`, { delay: 50 });
...
Cypress.Commands.add('selection', { prevSubject: true }, (subject: JQuery<HTMLElement>, fn: ($el: JQuery<HTMLElement>) => void) => {
  cy.wrap(subject)
    .trigger('mousedown')
    .then(fn)
    .trigger('mouseup');
...
Cypress.Commands.add('setSelection', { prevSubject: true }, (subject: JQuery<HTMLElement>, query: string | SelectionQuery, endQuery?: string) => {
...
Cypress.Commands.add('setCursor', { prevSubject: true }, (subject: JQuery<HTMLElement>, query: string, atStart?: boolean) => {
  return cy.wrap(subject).selection($el => {
