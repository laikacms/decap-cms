...
Route `@laikacms/decap-cms/ai`'s bearer verification through the `resolveBearer -> { user, scopes }`
...
whether from `decap-cms-lib-pat`'s `resolveBearer` in this repo or the parallel `laikacms/auth`
...
from `@laikacms/decap-cms/ai` so consumers do not need a separate `decap-cms-lib-pat` dependency to
