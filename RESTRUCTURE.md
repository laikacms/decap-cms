...
3. **`pnpm build`** (tsup). Produces `dist/<name>/index.js` and `dist/<name>/index.d.ts` for each
   entry, matching the `exports` map. The tsup config auto-discovers entries from
   `src/<name>/index.{ts,tsx}` — if a subpath in `exports` has no corresponding `index.*`, the build
   fails on that entry. Fix is either to add the file or remove the subpath from `exports`.
